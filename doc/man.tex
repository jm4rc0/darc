...
\author{Alastair Basden \\ Durham University \\ Department of Physics
...
Details of the architecture of darc can be found in \cite{basden9}.
...
\begin{itemize}
\item[-sPREFIX | --prefix=PREFIX] Specify a prefix (to create a unique
...
\begin{itemize}
\item[-b BUFSIZE | --bufsize=BUFSIZE] Specify the buffer size to be
  used locally
\item[-c configfile.py | --config=configfile.py] Specify the
  configuration file used to configure the GUI.
\item[-h | --help] Print a help message
\item[-n] Don't request comments when sending newly changed
  parameters.
\item[-s PREFIX | --prefix=PREFIX] The prefix for the RTC that you
  wish to connect to.
\end{itemize}
...
system on Palomar (cite SPIE vol 6272, Dekany et al) on a 2008 era PC,
