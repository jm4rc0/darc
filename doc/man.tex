...
\section{Python API}
Accessing and controlling darc from Python is straightforward using
the darc module:

\begin{verbatim}
import darc
#Connect to darc
d=darc.Control() # or darc.Control("prefix") if you have a prefix
...
#set a parameter:
d.Set("myParameter",numpy.zeros((100,),numpy.float32))
d.Set("myOtherParam","test")

#get a single frame of data
data=d.GetStream("rtcPxlBuf")  #returns (data,timestamp,frame number)
#Other streams can include rtcCalPxlBuf, rtcCentBuf, rtcMirrorBuf,
#rtcFluxBuf, rtcActuatorBuf, etc.

#get many frames of data
data=d.GetStreamBlock("rtcCentBuf",100,asArray=1)
#returns {"rtcCentBuf":(data,timestamp,frame number)}

#A callback can also be specified to process data on the fly:
def cb(data):
 print data
 return 0 #return !=0 to stop.
d.GetStreamBlock("rtcCentBuf",-1,callback=cb)

#Sum some data:
data=d.SumData("rtcPxlBuf",100,dtype="f") #sum 100 frames of raw images

#Get the decimation rates:
d.GetDecimation()

#Set decimation:
d.SetDecimation("rtcCentBuf",100)
...
A flag, whether the actuator data should be sent to the DM or not.
This might be better named: sendToDm.
...
values, or add (if 1) to them.  Turning this on will engage (close)
the AO loop.  This might be better named: engageLoop.
...
\subsection{corrUpdateGain}
A gain to use if you want to automatically update the correlation
reference images based on integrated images (using a rolling leaky
average with this gain).  Set to zero to turn off, and only turn on
when already in correlation mode.

\subsection{corrUpdateToCoG}
If in auto-update mode, specifies whether the integrated shift-add
image position is specified by CoG or correlation.  CoG seems to work
best, but is slightly more CPU-intensive.

\subsection{corrUpdateNfr}
If in auto-update mode, specifies the number of frames over which a
whole update of the correlation references will occur.  This can be
used to reduce computational load.  If set to 1, the correlation
reference for every sub-aperture will be updated every frame.  If set
to greater than 1, the correlation references for each sub-aperture
will be updated every N frames.  Note, the integrated images are still
integrated every frame.

\subsection{corrClipIntegImg}
If in auto-update mode, specifies whether to clip the integrated
image.  Probably should.

\subsection{corrImgOffset}
If in auto-update mode, specifies a CoG offset to apply to the
calibrated images when doing shift-add to create the integrated
images.  Similar principal to adapWinOffset.

\subsection{corrNStore}
Size of correlation-related circular buffers (number of entries).
...
\subsubsection{adapBoundary}
The absolute boundary for each camera, beyond which the adaptive
windows are not allowed to move.  If not specified, defaults to the
camera dimensions (pixels).  Specified as four 32-bit integers per
camera, which are ymin, ymax, xmin, xmax, with ymin and ymax
defaulting to zero.  This parameter can be used to avoid detector edge
effects.  
...
\subsubsection{adapWinOffset}
None, or an float32 array of size equal to the number of slopes
(i.e.\ twice the number of active sub-apertures).  If used, this
specifies an offset to be applied when calculating adaptive window
positions.  This necessary when sub-aperture images are not centred,
e.g.\ for highly elongated LGS spots with more weight at one end
(e.g.\ higher sodium density lower down).  Probably should not be used
in correlation mode (since you can just redefine the correlation
references to centre the spots).  

To compute this, set the sub-aperture images centrally within the
sub-apertures, and set the adapWinOffset values equal to the negative
of the raw slope measurements (i.e. without refCentroids set - or with
refCentroids added back on).
...
\subsection{Processor options}
Obviously, you should optimise your system, remove unnecessary
processes, set the CPU governor to performance etc.  e.g. on Fedora,
...
\subsection{darc and NUMA}
Currently, darc is not really NUMA aware (leaving the kernel to handle
everything).  This can limit performance on multi-CPU systems.

We are investigating possibilities.  However, one possible solution
would be to have a separate parameter buffer for each NUMA node,
allowing threads to access that particular buffer.  Maybe this can be
restricted in size, so that only specified parameters are placed there
(e.g.\ control matrices, calibration maps, reference slopes, etc),
with parameters automatically copied when a buffer swap is requested.
This however might introduce jitter.  Alternatively, have a double
buffered parameter buffer on each NUMA node, which are kept
synchronised with the master.  This will significantly increase memory
requirements.  

Another option, which is currently preferred, is to only enable this
feature when subapAllocation is used, i.e.\ each thread knows in
advance which sub-apertures it will be processing.  Then, each thread
can make its own local copy of the relevant parts of the necessary
memory, for example the control matrix and calibration images.
Additionally, if the thread affinity of a given thread is within the
thread affinity of the initial, main, thread (i.e.\ it will operate on
the processor to which the parameter buffers are attached), then this
copy is not necessary, i.e.\ it will continue to access the memory in
