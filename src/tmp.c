...
int camOpen(char *name,int n,int *args,char *buf,circBuf *rtcErrorBuf,char *prefix,arrayStruct *arr,void **camHandle,int npxls,short *pxlbuf,int ncam,int *pxlx,int* pxly,int* frameno){
  ...
  TEST(camstr->pxlShift=calloc(ncam,sizeof(int)));
...
int camWaitPixels(int n,int cam,void *camHandle){
  ...
  if(n>camstr->pxlsTransferred[cam]){
	...
	pxlno=camstr->npxlsArrCum[cam]+i+camstr->pxlShift[cam];
	...
	if(camstr->pxlRowStartSkipThreshold!=0 && ((i+camstr->pxlShift[cam])%camstr->pxlx[cam])==0 && camstr->imgdata[pxlno]<camstr->pxlRowStartSkipThreshold){
	  camstr->pxlShift[cam]--;//if the dark pixel is in first colum, need to remove a pixel.
	}else if(camstr->pxlRowEndInsertThreshold!=0 && ((i+camstr->pxlShift[cam])%camstr->pxlx[cam])==camstr->pxlx[cam]-2 && camstr->imgdata[pxlno]<camstr->pxlRowEndInsertThreshold){//If the dark pixel is in the 2nd last column, need to add a pixel (it should fall in the last column with the andors).
	  camstr->pxlShift[cam]++;
	...
	for(i=n-camstr->pxlShift[cam]; i<n; i++){
