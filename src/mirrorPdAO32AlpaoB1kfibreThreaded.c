...
int mirrorOpen(char *name,int narg,int *args,paramBuf *pbuf,circBuf *rtcErrorBuf,char *prefix,arrayStruct *arr,void **mirrorHandle,int nacts,circBuf *rtcActuatorBuf,unsigned int frameno,unsigned int **mirrorframeno,int *mirrorframenoSize){
  ...
  if(narg>1 && narg>args[0]+5){
    ...
    if(narg>=5+args[0]+mirstr->nboards+mirstr->nalpao*2+(mirstr->nboards+mirstr->nalpao)*2+mirstr->nbmm*2){
      //alloc some memory to put it all in...
      if((mirstr->boardNumber=calloc(sizeof(int),mirstr->nboards+2*mirstr->nalpao+(mirstr->nboards+mirstr->nalpao)*2+mirstr->nbmm*2))==NULL){
	...
	*mirrorHandle=NULL;
	return 1;
      ...
      mirstr->nactInit=&mirstr->boardNumber[mirstr->nboards*2+mirstr->nalpao*3+mirstr->nbmm];
      memcpy(mirstr->boardNumber,&args[5+args[0]],sizeof(int)*(mirstr->nboards*3+mirstr->nalpao*4+mirstr->nbmm*2));
      for(i=0;i<mirstr->nalpao;i++){
	...
	printf("Using mirror %s\n",(char*)(&mirstr->alpaoSerialNo[i*2]));
      ...
      printf("Error: wrong number of args - should be Naffin, prio, affin[Naffin], nPD32AOboards,nalpao,nbmm, boardNumber[board],boardNumber[board],...,serialNo[alpao(8 bytes)],serialNo[alpao(8bytes)], nacts[board/alpao/bmm],nacts[board/alpao/bmm],...nactInit[board/alpao/bmm],nactInit[board/alpao/bmm],..., \n(alpao serial numbers are 6 chars long, and here should be packed into 2 ints)");
      mirrordofree(mirstr);
      *mirrorHandle=NULL;
      return 1;
    }
  ...
  for(i=mirstr->nboards+mirstr->nalpao;i<mirstr->nboards+mirstr->nalpao+mirstr->nbmm;i++){
    mirstr->nactsBMM+=mirstr->nactBoard[i];
    printf("DM %d: %d\n",i,mirstr->nactBoard[i]);
  }
  mirstr->totactBoard=mirstr->nactsBoard+mirstr->nactsAlpao+mirstr->nactsBMM;

  mirstr->arrsize=(mirstr->nactsBoard+mirstr->nactsBMM)*sizeof(unsigned short);
...
int mirrorSend(void *mirrorHandle,int n,float *data,unsigned int frameno,double timestamp,int err,int writeCirc){
  ...
  if(err==0 && mirstr!=NULL && mirstr->open==1){
	...
	for(i=nactsBoard+nactsAlpao; i<nactsBoard+nactsAlpao+nactsBMM; i++){
	  intDMCommand=(int)(data[i]+0.5);
	  mirstr->arr[i-nactsAlpao]=(unsigned short)intDMCommand;
	  if(intDMCommand<mirstr->actMin[i]){
	    nclipped++;
	    mirstr->arr[i-nactsAlpao]=mirstr->actMin[i];
	  }
	  if(intDMCommand>mirstr->actMax[i]){
	    nclipped++;
	    mirstr->arr[i-nactsAlpao]=mirstr->actMax[i];
	  }
	}
    ...
    if(writeCirc){
      ...
      circInsert(mirstr->rtcActuatorBuf,&mirstr->arr[nactsBoard],sizeof(unsigned short)*mirstr->nactsBMM,sizeof(unsigned short)*mirstr->nactsBoard+sizeof(double)*mirstr->nactsAlpao);
...
int mirrorNewParam(void *mirrorHandle,paramBuf *pbuf,unsigned int frameno,arrayStruct *arr){
  ...
  if(err==0){
    ...
    if(mirstr->rtcActuatorBuf!=NULL && mirstr->rtcActuatorBuf->datasize!=(mirstr->nactsBoard+mirstr->nactsBMM)*sizeof(unsigned short)+mirstr->nactsAlpao*sizeof(double)){
      int tmp=((mirstr->nactsBoard+mirstr->nactsBMM)*sizeof(unsigned short)+mirstr->nactsAlpao*sizeof(double))/sizeof(unsigned short);
