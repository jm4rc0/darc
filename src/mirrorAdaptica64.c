...
This is written for the adaptica AO64 driver.

Unfortunately, adaptiva .so library is 32 bit, so we have to run it as a separate process and communicate with shm.
...
void mirrordofree(MirrorStruct *mirstr){
  ...
  if(mirstr!=NULL){
    ...
    unlink("/tmp/adaptica");
    if(mirstr->fd>0){//wait it up.
      i=write(mirstr->fd,&i,sizeof(int));
      close(mirstr->fd);
    }
...
int initDM(MirrorStruct *mirstr){
  int fd,i;
  struct stat st;
  if((mirstr->dmarr=(float*)calloc(66,sizeof(float)))==NULL){//66 since zeropadded at both ends.
    printf("Error - unable to alloc dmarr: %s\n",strerror(errno));
    return 1;
  }
  //first, spawn the process.
  if(system("mirrorAdapticaProcess &")!=0){
    printf("Error starting mirrorAdapticaProcess\n");
    return 1;
  }
  //now wait for /tmp/adaptica to appear.
  i=0;
  while(i<10){
    if(stat("/tmp/adaptica",&st)==0)
      break;
    i++;
    sleep(1);
  }
  if(i==10){
    printf("Failed to open /tmp/adaptica fifo\n");
    return 1;
  }
  if((fd=open("/tmp/adaptica",O_WRONLY))==-1){
...
inline void dmWrite(MirrorStruct *mirstr,int indx,float val){
  if(indx>=64)
    printf("Error setting actuator %d - continuing\n",indx);
  else
    mirstr->dmarr[indx+1]=val;
}

int dmSend(MirrorStruct *mirstr){
  int err=0,n;
  if((n=write(mirstr->fd,mirstr->dmarr,sizeof(float)*66))!=sizeof(float)*66){
    printf("Error writing mirror values - %d/%d\n",n,(int)sizeof(float)*66);
    err=1;
  }
  return err;
...
void* worker(void *mirstrv){
  ...
  while(mirstr->open){
	...
	for(j=0;j<mirstr->oscillateIters;j++){
	  ...
	  if(mirstr->actMapping==NULL){
	    for(i=0;i<mirstr->nacts;i++){
	      val=(mirstr->arr[i]+mirstr->oscillateArr[j*skip+step*i]*(mirstr->arr[i]-mirstr->arrPrev[i]));
	...
	memcpy(mirstr->arrPrev,mirstr->arr,sizeof(float)*nel);
      }
    }
  }
...
int mirrorOpen(char *name,int narg,int *args,paramBuf *pbuf,circBuf *rtcErrorBuf,char *prefix,arrayStruct *arr,void **mirrorHandle,int nacts,circBuf *rtcActuatorBuf,unsigned int frameno,unsigned int **mirrorframeno,int *mirrorframenoSize){
  ...
  mirstr->arrsize=nacts*sizeof(float);
...
int mirrorSend(void *mirrorHandle,int n,float *data,unsigned int frameno,double timestamp,int err,int writeCirc){
  ...
  if(err==0 && mirstr!=NULL && mirstr->open==1){
    ...
    if(mirstr->actMapping==NULL){
      for(i=0; i<mirstr->nacts; i++){
	mirstr->arr[i]=data[i];
	if(data[i]<mirstr->actMin[i]){
	  nclipped++;
	  mirstr->arr[i]=mirstr->actMin[i];
	}
	if(data[i]>mirstr->actMax[i]){
	  nclipped++;
	...
	if(mirstr->actScale==NULL){
	  if(mirstr->actOffset==NULL){
	    ...
	    for(i=0; i<mirstr->actMappingLen; i++){
	      mirstr->arr[i]=data[i]+mirstr->actOffset[i];
	      if(mirstr->arr[i]<mirstr->actMin[i]){
		nclipped++;
		mirstr->arr[i]=mirstr->actMin[i];
	      }
	      if(mirstr->arr[i]>mirstr->actMax[i]){
		nclipped++;
		mirstr->arr[i]=mirstr->actMax[i];
	      }
	    }
	  ...
	  if(mirstr->actOffset==NULL){
	    for(i=0; i<mirstr->actMappingLen; i++){
	      mirstr->arr[i]=data[i]*mirstr->actScale[i];
	      if(mirstr->arr[i]<mirstr->actMin[i]){
		nclipped++;
		mirstr->arr[i]=mirstr->actMin[i];
	      }
	    ...
	    for(i=0; i<mirstr->actMappingLen; i++){
	      mirstr->arr[i]=data[i]*mirstr->actScale[i]+mirstr->actOffset[i];
	      if(mirstr->arr[i]<mirstr->actMin[i]){
		nclipped++;
		mirstr->arr[i]=mirstr->actMin[i];
	      }
	...
	if(mirstr->actScale==NULL){
	  if(mirstr->actOffset==NULL){
	    for(i=0; i<mirstr->actMappingLen; i++){
	      mirstr->arr[i]=data[mirstr->actSource[i]];
	      if(mirstr->arr[i]<mirstr->actMin[i]){
		nclipped++;
		mirstr->arr[i]=mirstr->actMin[i];
	      }
	    ...
	    for(i=0; i<mirstr->actMappingLen; i++){
	      mirstr->arr[i]=data[mirstr->actSource[i]]+mirstr->actOffset[i];
	      if(mirstr->arr[i]<mirstr->actMin[i]){
		nclipped++;
		mirstr->arr[i]=mirstr->actMin[i];
	      }
	  ...
	  if(mirstr->actOffset==NULL){
	    for(i=0; i<mirstr->actMappingLen; i++){
	      mirstr->arr[i]=data[mirstr->actSource[i]]*mirstr->actScale[i];
	      if(mirstr->arr[i]<mirstr->actMin[i]){
		nclipped++;
		mirstr->arr[i]=mirstr->actMin[i];
	      }
	    ...
	    for(i=0; i<mirstr->actMappingLen; i++){
	      mirstr->arr[i]=data[mirstr->actSource[i]]*mirstr->actScale[i]+mirstr->actOffset[i];
	      if(mirstr->arr[i]<mirstr->actMin[i]){
		nclipped++;
		mirstr->arr[i]=mirstr->actMin[i];
	      }
