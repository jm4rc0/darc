...
#define NUM_OF_BUFFER 16 // Buffers used internally by libgenicam
	 ...
	 if (iWaitResult == J_COND_WAIT_SIGNAL) {
	   err=0;
	    ...
	    if((ngot%1000)==0){
	      ...
	      t1=t2;
	      printf("jaicam freq %g, ngot=%d, nskipped=%d img size %d\n",freq,ngot,nskipped,tAqImageInfo.iImageSize);
	    }
	    if (m_bEnableThread) {
...
int camOpen(char *name, int n, int *args, void **camHandle,
	    int npxls, short *pxlbuf, int ncam, int *pxlx, int *pxly, int *frameno, char *buf, circBuf *rtcErrorBuf)
{
   CamStruct *camstr;
