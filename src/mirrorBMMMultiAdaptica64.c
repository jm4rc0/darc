...
This is written for the boston micromachines multi DM and adaptica AO64 driver.

Unfortunately, adaptiva .so library is 32 bit, so we have to run it as a separate process and communicate with shm.

First 48 values are assumed to be for the adaptica, and the rest (160) are for the BMM multi DM.
...
int initDM(MirrorStruct *mirstr){
  ...
  if(mirstr->udev!=NULL){//now open the adaptica
  int fd,i;
  struct stat st;
...
int dmSend(MirrorStruct *mirstr){
  int err=0,rtval,n;
  if((rtval=usb_bulk_write(mirstr->udev,2,(char*)mirstr->dmarr,NUM_ACTUATORS*sizeof(unsigned short),TIMEOUT))!=USB_BYTES_PER_FRAME_MULTI){
    printf("Sending actuators failed - written %d bytes <%s>\n",rtval,usb_strerror());
    err=1;
  }
  if((n=write(mirstr->fd,mirstr->adarr,sizeof(float)*66))!=sizeof(float)*66){
