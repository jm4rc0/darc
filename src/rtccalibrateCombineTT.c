...
/*
This is a special case of rtccalibrate, which sums the raw pixels of selected sub-apertures to give a mean signal that is better, and thus improves snr before brightest pixel selection.  This allows a wfs to turn into a more sensitive tt sensor.

Notes:  All the subaps to be combined should be combined into the last one of these
*/
...
int sumRawSubaps(CalStruct *cstr,int cam,int wfs,int threadno,int ndoing){
  //sum subaps together to mimic tip-tilt (only a good approximation for a low noise camera).
  //First of all, sum into subap.  Then scale to less than uint16, and then duplicate back across the raw image.
  CalThreadStruct *tstr=cstr->tstr[threadno];
  float *subap=tstr->subap;
  int npxlx=cstr->npxlx[cam];
  int *loc;
  int cursubindx=tstr->cursubindx;//this will start with the first valid subap to use.
  int k,nx=0,ny=0,i,j,cnt;
  unsigned short *Hpxlbuf;
  float *fpxlbuf;
  float mx;
  int scale;
  cursubindx+=wfs;
...
int calibrateNewParam(void *calibrateHandle,paramBuf *pbuf,unsigned int frameno,arrayStruct *arr){
  ...
  if(err==0){
    ...
    cstr->combineTT=0;
    if(index[COMBINETT]>=0){
      if(dtype[COMBINETT]=='i' && nbytes[COMBINETT]==sizeof(int)){
	cstr->combineTT=*(int*)values[COMBINETT];
      }else{
	printf("combineTT error\n");
	err=1;
      }
    }
...
int calibrateNewSubap(void *calibrateHandle,int cam,int threadno,int cursubindx,float **subap,int *subapSize,int *NProcessing,int *rubbish){//subap thread
  ...
  if(nprocessing==94){
    ...
    if((cstr->combineTT>>(cam*2))&0x1){
      //first wfs on this camera should be summed...
      sumRawSubaps(cstr,cam,0,threadno,nprocessing);
    }
    if((cstr->combineTT>>(cam*2+1))&0x1){
      //second wfs on this camera should be summed...
      sumRawSubaps(cstr,cam,1,threadno,nprocessing);
    }
  }
