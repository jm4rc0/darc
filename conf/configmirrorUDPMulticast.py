...
addr="224.1.1.1;127.0.0.1"#multicast on localhost
#addr="129.234.187.90"#udp.
#addr="127.0.0.1"
addr+"\0"
while len(addr)%4!=0:
    addr+="\0"
mirrorParams=numpy.zeros((7+len(addr)//4,),"i")
mirrorParams[0]=1500-128#payload - I think MTU size-28 (for the IP/UDP headers)
mirrorParams[1]=port#port
mirrorParams[2]=1#thread affinity el size
mirrorParams[3]=1#thread prioirty
mirrorParams[4]=-1#thread affinity
mirrorParams[5]=0#don't send the prefix.
mirrorParams[6]=1#asfloat
mirrorParams[7:]=numpy.fromstring(addr,dtype="i")
...
control={
    ...
    "actMin":(numpy.ones((nacts,),numpy.float32)*(-65535)).astype(numpy.float32),#4095,#max actuator value
    "nacts":nacts,
    "ncam":ncam,
    "nsub":nsub,
    "npxly":npxly,
    ...
    "nsteps":0,
    "addActuators":1,
    "actuators":numpy.zeros((nacts,),numpy.float32),#(numpy.random.random((3,52))*1000).astype("H"),#None,#an array of actuator values.
