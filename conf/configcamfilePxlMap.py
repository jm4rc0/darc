...
overlap=2#how much the subaps overlay.
...
for k in range(ncam):
    for i in range(nsuby[k]):
        for j in range(nsubx[k]):
            ...
            if subapFlag[indx]:
                subapLocation[indx]=(8+i*suby[k]-overlap,8+i*suby[k]+suby[k]+overlap,1,8+j*subx[k]-overlap,8+j*subx[k]+subx[k]+overlap,1)
...
#Set a pixel map - the lazy way... idendical for each subap.
pxlMap=numpy.zeros((nsub.sum()+2+(subx.max()+2*overlap)*(suby.max()+2*overlap),),numpy.int32)
pxlMap[:nsub.sum()]=nsub.sum()#the offset is the same for each subap.
pxlMap[nsub.sum()]=suby.max()+2*overlap
pxlMap[nsub.sum()+1]=subx.max()+2*overlap
tmp=pxlMap[nsub.sum()+2:].view(numpy.float32)
tmp.shape=suby.max()+2*overlap,subx.max()+2*overlap
m=max(suby.max()+2*overlap,subx.max()+2*overlap)
tmp[:]=tel.Pupil(m,m/2,0).fn[:suby.max()+2*overlap,:subx.max()+2*overlap]
