...
subapFlag=numpy.zeros(((nsuby*nsubx).sum(),),numpy.int32)
subapFlag[:]=1
...
ystep=1
xstep=2
subx=(npxlx-16*xstep)/nsubx*xstep
suby=(npxly-16*ystep)/nsuby*ystep
for k in range(ncam):
...
#    "correlationPSF":correlationPSF,
    ...
    "figureParams":numpy.array([1000,0,0xffff,1]).astype("i"),#timeout,port,affin    
    }
#set the gain array
#control["gain"][:2]=0.5
