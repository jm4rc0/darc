...
def SerialiseToList(value,sendList=[]):
    ...
    """
    thisType = type(value)
    if thisType in [types.IntType,numpy.int32,type(numpy.array([0]).astype("i").view("i")[0])]:
        value = numpy.array(value, numpy.int32).tostring()#"i"
    elif thisType in [types.FloatType,numpy.float64]:
        value = numpy.array(value, numpy.float64).tostring()#"d"
    elif thisType==numpy.float32:
    ...
    elif thisType in [types.IntType,numpy.int32,type(numpy.array([0]).astype("i").view("i")[0])]:#added by agb - the last one is necessary for 32 bit machines where numpy screws up...
        headerByte=translate.index(types.IntType)
        length=len(value)#value.itemsize
    elif thisType in [types.FloatType,numpy.float64]:#added by agb
        headerByte=translate.index(types.FloatType)
        length=len(value)#value.itemsize
    elif thisType==numpy.float32:
        headerByte=translate.index(numpy.float32)
